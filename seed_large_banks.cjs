...
const SubjectQuestion = require('../backend/models/SubjectQuestion'); // We will run this from knowledge-hub-pro-main
require('dotenv').config({ path: './backend/.env' });

// Procedural Question Generators to construct 100 unique questions per bank per subject
const generateCQuestions = (bank) => {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        // Random theory questions injected every 4 items
        if (i % 4 === 0) {
            const theoryQ = [
            ...
            qText = theoryQ.q + ` (Bank ${bank} Variation ${i})`;
...
const runMigration = async () => {
    try {
        ...
        // Generate exactly 100 questions per Bank (1, 2, 3) for each subject
        ['C', 'Python', 'Java'].forEach(subject => {
            [1, 2, 3].forEach(bank => {
                console.log(`Generating 100 questions for Subject: ${subject}, Bank: ${bank}...`);
                if (subject === 'C') allQuestionsToInsert.push(...generateCQuestions(bank));
                if (subject === 'Python') allQuestionsToInsert.push(...generatePythonQuestions(bank));
                if (subject === 'Java') allQuestionsToInsert.push(...generateJavaQuestions(bank));
            });
        });

        console.log(`Total questions generated: ${allQuestionsToInsert.length} (Expected: 900)`);
        
        console.log("Inserting batch into SubjectQuestion collection directly...");
        // Directly insert all 900 into existing DB collection without clearing!
        await SubjectQuestion.insertMany(allQuestionsToInsert);
        
        console.log("✅ Success! Database populated heavily with new mixed questions without deleting existing data.");
        process.exit(0);
    } catch (e) {
